...
Namespace ModelDevelopment.UserControls
    Friend NotInheritable Class UC_RESULTS
        ...
        Private MY_DISPLAYRACE As CascadeServer.UserControls.UC_DISPLAYRACE
        Private MY_UI_MODEL_TOTALS As CascadeServer.UserControls.UC_Model_Totals
        ...
        Friend Sub New()
            ...
            MY_UI_MODEL_TOTALS = New CascadeServer.UserControls.UC_Model_Totals()
        ...
        End Sub
        Friend Sub Populate(ByVal DataTable As Data.DataTable, ByVal StartDate As String, ByVal FinishDate As String, ByVal TabPage As UserControls.UC_TABPAGE)
            ...
            For Each Row As Data.DataRow In DataTable.Select("", "MEETING_INFO_OADATE DESC")
                ...
                Using DGVRow As New DataGridViewRow
                    ...
                    If Not IsDBNull(Row.Item("HASMODEL")) AndAlso CBool(Row.Item("HASMODEL")) Then
                        Call CascadeServer.UserControls.GridViewCellStyles.AddDefaultCellStyleToCells(DGVRow, Me.DGV_RESULTS)
                        For Each Cell As DataGridViewCell In DGVRow.Cells
                            ...
                            If Me.DGV_RESULTS.Columns(Cell.ColumnIndex).Visible = True Then
                                Cell.Style = CascadeServer.UserControls.GridViewCellStyles.HasModel(Cell, Me.DGV_RESULTS.Columns(Cell.ColumnIndex).DefaultCellStyle)
                            End If
                        Next
                    End If
                    '
                    Select Case WIN
                        Case Is < 0
                            Call CascadeServer.UserControls.GridViewCellStyles.ChangeForeColor(DGVRow, Me.DGV_WINS.Index, Color.Red)
                        Case Is >= 1
                            Call CascadeServer.UserControls.GridViewCellStyles.ChangeForeColor(DGVRow, Me.DGV_WINS.Index, Color.Green)
                        Case Else
                            Call CascadeServer.UserControls.GridViewCellStyles.ChangeForeColor(DGVRow, Me.DGV_WINS.Index, Color.Black)
                    End Select
                    Select Case PLACE
                        Case Is < 0
                            Call CascadeServer.UserControls.GridViewCellStyles.ChangeForeColor(DGVRow, Me.DGV_PLACES.Index, Color.Red)
                        Case Is >= 1
                            Call CascadeServer.UserControls.GridViewCellStyles.ChangeForeColor(DGVRow, Me.DGV_PLACES.Index, Color.Green)
                        Case Else
                            Call CascadeServer.UserControls.GridViewCellStyles.ChangeForeColor(DGVRow, Me.DGV_PLACES.Index, Color.Black)
                    End Select
                    '
                    If Not RowAdd Then
                        Call CascadeServer.UserControls.GridViewCellStyles.AddDefaultCellStyleToCells(DGVRow, Me.DGV_RESULTS)
                        Call CascadeServer.UserControls.GridViewCellStyles.MakeRowScratched(1, DGVRow, Me.DGV_RESULTS)
                    End If
                    RowCount += 1
                End Using
            Next
            ...
            '
            Call StoredProcedures.ModelExplorer.AddNewModelLog(TabPage.Properties.ID, LogResults, Me._Connection)
            '
        ...
        Private Sub DGV_RESULTS_CellMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles DGV_RESULTS.CellMouseClick
            ...
            Select Case e.ColumnIndex
                Case Me.DGV_NAME.Index ' Name of the race
                    ...
                    MY_DISPLAYRACE = New CascadeServer.UserControls.UC_DISPLAYRACE(PARAMETERS(0), PARAMETERS(1), PARAMETERS(2), PARAMETERS(3), PARAMETERS(4), Me._Connection)
