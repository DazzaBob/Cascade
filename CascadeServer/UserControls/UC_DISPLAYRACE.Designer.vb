Namespace UserControls
    ...
    Partial Class UC_DISPLAYRACE
        ...
        Private Sub InitializeComponent()
            ...
            TPRACERUNNERS = New TabPage()
            TC_RACE_RUNNERS = New TabControl()
            TP_RACERUNNERS_MAIN = New TabPage()
            TC_RACERUNNERS_MAIN = New TabControl()
            TP_RACERUNNERS_MAIN_DISTANCE = New TabPage()
            SC_RACERUNNERS_MAIN_DISTANCE = New SplitContainer()
            GB_RACERUNNERS_MAIN_DISTANCE = New GroupBox()
            TP_RACERUNNERS_MAIN_GROUP = New TabPage()
            SC_RACERUNNERS_MAIN_GROUP = New SplitContainer()
            GB_RACERUNNERS_MAIN_GROUP = New GroupBox()
            TP_RACERUNNERS_MAIN_OVERALL = New TabPage()
            SC_RACERUNNERS_MAIN_OVERALL = New SplitContainer()
            GB_RACERUNNERS_MAIN_OVERALL = New GroupBox()
            TP_RACERUNNERS_RACEBARRIER = New TabPage()
            TP_RESULTS = New TabPage()
            TP_MODELS = New TabPage()
            TP_BARRIER = New TabPage()
            TC_BARRIER = New TabControl()
            TP_BARRIER_ALL = New TabPage()
            TP_BARRIER_90DAYS = New TabPage()
            TP_BARRIER_180DAYS = New TabPage()
            TP_BARRIER_270DAYS = New TabPage()
        ...
        Private WithEvents TPRACERUNNERS As TabPage
        Private WithEvents TP_RESULTS As TabPage
        Private WithEvents TP_MODELS As TabPage
        Private WithEvents TP_BARRIER As TabPage
        Private WithEvents TC_RACE_RUNNERS As TabControl
        Private WithEvents TP_RACERUNNERS_MAIN As TabPage
        Private WithEvents TC_RACERUNNERS_MAIN As TabControl
        Private WithEvents TP_RACERUNNERS_MAIN_DISTANCE As TabPage
        ...
        Private WithEvents TP_RACERUNNERS_RACEBARRIER As TabPage
        Private WithEvents TC_BARRIER As TabControl
        Private WithEvents TP_BARRIER_ALL As TabPage
        Private WithEvents TP_BARRIER_90DAYS As TabPage
        Private WithEvents TP_BARRIER_180DAYS As TabPage
        Private WithEvents TP_BARRIER_270DAYS As TabPage
    End Class
End Namespace