        ...
        Private NotInheritable Class XmlLoader : Implements IDisposable
            ...
            Friend Function ListOfMeetingInformation() As List(Of Instance.MeetingInformation)
                ...
                For Each Meeting As String In SplitMeetings
                    ...
                    '
                    THIS_STOPWATCH.Stop()
                    Call ServerUI.Messages.LogMessages("Loaded meeting: M" & CStr(Number) & " - " & Name & " to memory in: " & Cascade.GetTime(THIS_STOPWATCH) & ".", Cascade.BroadcastTypes.Log)
                Next Meeting
                '
            ...
            Private Shared Sub SetVenue(ByVal Xml As String, ByRef Venue As String)
                ...
                End If
            End Sub
            Private Shared Sub UpdateVenue(ByRef MeetingInformation As List(Of Instance.MeetingInformation))
                ...
                Using DT As Data.DataTable = GetDataTable
                    For Each MEETING As Instance.MeetingInformation In MeetingInformation
                        For Each RACE As Instance.MeetingInformation.RaceInformation In MEETING.ListOfRaceInformation
                            ...
                            Dim DR() As Data.DataRow = DT.Select("VENUE='" & RACE.VENUE & "'")
                            If DR.Length > 0 Then
                                RACE.VENUE = CStr(DR(0).Item("MAPTOVENUE"))
                            End If
                            '
                            DR = DT.Select("(COUNTRY='" & MEETING.COUNTRY & "') AND (VENUE='" & THIS_VENUE & "')")
                            If DR.Length > 0 Then
                                MEETING.COUNTRY = CStr(DR(0).Item("MAPTOCOUNTRY"))
                                MEETING.VENUE = CStr(DR(0).Item("MAPTOVENUE"))
                                RACE.VENUE = CStr(DR(0).Item("MAPTOVENUE"))
                            End If
                        Next RACE
                    Next MEETING
                End Using
            End Sub
            Private Shared ReadOnly Property GetDataTable() As Data.DataTable
                Get
