...
Namespace ModelDevelopment
    Partial Class FORM_MODELDEVELOPMENT
        ...
        Private Sub TV_MODELS_KeyUp(sender As Object, e As KeyEventArgs) Handles TV_MODELS.KeyUp
            Select Case e.KeyCode
                Case Keys.Delete
                    ...
                    Select Case DirectCast(TN.Tag, NodeTypes)
                        Case NodeTypes.Folder
                            Call CMS_TV_DELETEFOLDER_Click(sender, e)
                            Return
                        Case NodeTypes.BaseFolder
        ...
        Private Sub TVMain_DragDrop(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles TV_MODELS.DragDrop
            ...
            If CType(This_Target_Drop_Node.Tag, NodeTypes) = NodeTypes.Model Then e.Effect = DragDropEffects.None : Return
            If CType(This_Move_This_Node.Tag, NodeTypes) = NodeTypes.BaseFolder Then e.Effect = DragDropEffects.None : Return
            If This_Move_This_Node Is This_Target_Drop_Node Then e.Effect = DragDropEffects.None : Return
            '
            Me.Cursor = Cursors.WaitCursor
            If (CType(This_Move_This_Node.Tag, NodeTypes) = NodeTypes.Folder) Then
                ' Move a folder
            ...
            ElseIf (CType(This_Move_This_Node.Tag, NodeTypes) = NodeTypes.Model) AndAlso (CType(This_Target_Drop_Node.Tag, NodeTypes) = NodeTypes.Folder OrElse CType(This_Target_Drop_Node.Tag, NodeTypes) = NodeTypes.BaseFolder) Then
        ...
        Private Sub MoveModelTreeNode(ByRef TargetNode As TreeNode, ByRef CurrentNode As TreeNode)
            If Me.TC_Main.TabCount > 0 Then
                For Each TP As UserControls.TabPage In Me.TC_Main.TabPages
        ...
        Private Sub TVMain_AfterLabelEdit(sender As Object, e As NodeLabelEditEventArgs) Handles TV_MODELS.AfterLabelEdit
            ...
            Me.Cursor = Cursors.WaitCursor
            '
            Select Case CType(e.Node.Tag, NodeTypes)
                Case NodeTypes.Folder
        ...
        Private Sub TVMain_AfterSelect(sender As Object, e As TreeViewEventArgs) Handles TV_MODELS.AfterSelect
            ...
            Select Case CType(Me.TV_MODELS.SelectedNode.Tag, NodeTypes)
                Case NodeTypes.Model, NodeTypes.Clone
                    ...
                    End If
                    Exit Select
                Case NodeTypes.Folder, NodeTypes.BaseFolder
                    Exit Select
            End Select
            '
            Me.Cursor = Cursors.Default
        ...
        Private Sub TV_Main_AfterSelect_FindCreateModelTabPage(e As TreeViewEventArgs)
            ...
            If Not PageFound Then
                Me.TC_Main.TabPages.Add(New UserControls.TabPage(CStr(e.Node.Name), Me))
