...
#End Region
        ...
        Private Sub TSB_TESTCOMPILE_Click(sender As Object, e As EventArgs) Handles TSB_TESTCOMPILE.Click
            If MessageBox.Show("This will test compile all models and open the first one found with an error for modification, if any errors are found, are you sure you want to continue?", "Are you sure?", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2, MessageBoxOptions.DefaultDesktopOnly, False) = Windows.Forms.DialogResult.Cancel Then Exit Sub
            '
            Me.Cursor = Cursors.WaitCursor
            ...
            If DR.Length > 0 Then
                ...
                For Each ROW As Data.DataRow In DR
                    ...
                    If RuntimeCompiler.CompileCode(Me, Nothing, RuntimeCompiler.ModelExplorerDR(Connection, ROW.Item("MODEL_EXPLORER_ID").ToString), Connection) Is Nothing Then
                        Me.TC_Main.TabPages.Add(New UserControls.TabPage(ModelExplorerID, Me))
                        Exit For
                    End If
