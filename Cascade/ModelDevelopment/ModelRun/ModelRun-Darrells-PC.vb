Namespace ModelDevelopment
    Friend NotInheritable Class ModelRun : Implements IDisposable
        ...
        Friend Sub New(ByVal StartDate As String, ByVal FinishDate As String, ByVal ModelID As String, ByVal Connection As Connection, _
            ...
            If ModelExplorerDR Is Nothing Then
                MY_MODEL_DR = RuntimeCompiler.ModelExplorerDR(Connection, ModelID)
            Else
                ReDim MY_MODEL_DR(0)
            ...
            '
            MY_COMPILERRESULTS = RuntimeCompiler.CompileCode(Nothing, Nothing, MY_MODEL_DR, Connection)
        End Sub
        ...
        Friend Function GetResults_DT() As Data.DataTable
            ...
            Using NewCollectRecords As New RecordCollection(THIS_FILTER, MY_CONNECTION, MY_TSS_LABEL)
                ...
                Else
                    Call ServerUI.Messages.LogMessages(THIS_LABEL, Cascade.BroadcastTypes.Log)
                End If
                Dim Y As Int32 = 0
                ' 
                ' This should give us a fake table to populate the models into
                This_Model_Result_DT = Cascade.StoredProcedures.UC_RUNNERRACELIST.GetDataTable("MEETING_INFO.OADATE = 0", MY_CONNECTION)
                '
                Y = 0
                ...
                For Each Row As Data.DataRow In NewCollectRecords.RaceInfoDT.Select("")
                    ...
                    Using NewRace As New Race(NewCollectRecords)
                        ...
                        '
                        If MY_MODEL_DR.Length > 1 Then
                            Dim NewLiveModels As New LiveModels(MY_MODEL_DR, MY_CONNECTION, MY_COMPILERRESULTS)
        ...
        ByVal EI_O_DT As Data.DataTable, ByVal EI_O_S_DT As Data.DataTable, ByVal EI_O_T_DT As Data.DataTable, ByVal EI_O_T_S_DT As Data.DataTable)
            ...
            If Not Rslt Is Nothing Then
                ...
                If sRESULT.Contains("EXCEPTION") Then
                    ...
                    If MessageBox.Show(Text, Cascade.UnExpectedError, MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1, MessageBoxOptions.DefaultDesktopOnly, False) = DialogResult.No Then CancelModelRun = True
                    Exit Sub
                End If
                sRESULT = Nothing
                '
                ...
                If (Not Values Is Nothing) AndAlso (Values.Length > 0) Then
                    For Each Value As String In Values
                        ...
                        Dim DR As Data.DataRow = Cascade.StoredProcedures.UC_RUNNERRACELIST.GetDataRow(CStr(RaceInfoRow.Item("MEETING_INFO_OADATE")), CStr(RaceInfoRow.Item("MEETING_INFO_NUMBER")), CStr(RaceInfoRow.Item("NUMBER")), Value, FORM_MODELDEVELOPMENT.Connection)
